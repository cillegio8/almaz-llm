...
export default function MessageBubble({ message }: MessageBubbleProps) {
  ...
  return (
    ...
    >
      {!isUser && (
        <div className="w-7 h-7 bg-gold rounded-full flex items-center justify-center flex-shrink-0 mr-2 mt-1">
      ...
      <div
        className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
          isUser
            ? 'bg-midnight-lighter text-pearl border border-midnight-lighter/50 rounded-br-sm'
            : 'bg-midnight-light text-silver shadow-sm border border-midnight-lighter rounded-bl-sm'
        }`}
      >
